...
if generate == 1:
    ...
    # Fit the model
    num_examples = 100000
    model_linear = SVC(kernel='linear', class_weight='balanced', tol=0.00001*num_examples, max_iter=5000)
    model_linear.fit(normalized_train[0:num_examples], train_output[0:num_examples])

    # Evaluate over train set
    eval_examples = 10000
    acc = model_linear.score(normalized_train[0:eval_examples], test_output[0:eval_examples])
    print ("Accuracy on Train Set: {0}".format(acc))
    train_predictions = model_linear.predict(normalized_train[0:eval_examples])
    f1 = f1_score(train_output[0:eval_examples], train_predictions, average='macro')
    ...
    f1 = f1_score(validation_output[0:eval_examples], val_predictions, average='macro')
    ...
    f1 = f1_score(test_output[0:eval_examples], test_predictions, average='macro')
